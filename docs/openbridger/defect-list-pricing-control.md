...
| 编号 | 修复行为 | 自动化验证 |
|---|---|---|
| PC-COST-001 | 报价和成本快照新增必填 `upstream_group_ratio`；采购成本按原始报价乘以上游结算倍率计算。旧记录缺少倍率时进入风险列表，不再生成方案 | 新增 6.8 倍真实成本计算回归用例；服务层定价用例通过 |
| CONC-001 | 审批时重新确认主、备报价仍启用且未过期 | 过期后审批返回 `ErrPricingProposalStale` |
| CONC-002 | 审批时比较当前价格版本与方案版本 | 版本变化后审批返回 `ErrPricingProposalStale` |
| EDGE-001 | 主渠道输入或输出成本不为正时拒绝生成方案，并进入风险列表 | 零成本用例返回 `ErrPricingCostInvalid` |
| EDGE-013 | 先校验整批数据，再在同一数据库事务中写入报价及快照 | 非法批次写入 0 条 |

PC-COST-001 的代码缺陷已修复；由于本次没有使用真实上游 API Key 产生账单，仍需在生产验收时核对一次 OpenBridger 记录的采购成本与上游账单增量。
