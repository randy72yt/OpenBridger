...
原报告保留 2026-09-16 的缺陷发现结果。本节记录修复后的复验结果。

| 项目 | 结果 |
|---|---|
| 服务层定价与 QA 回归 | 通过 |
| PC-COST-001 上游倍率 6.8 成本换算 | 通过，0.22/0.66 转换为 1.496/4.488 |
| CONC-001 过期报价审批 | 通过，审批被拒绝 |
| CONC-002 价格版本变化审批 | 通过，审批被拒绝 |
| EDGE-001 主渠道零成本 | 通过，不生成可审批方案 |
| EDGE-013 非法批量导入 | 通过，整批写入 0 条 |
| SQLite / MySQL 8.0 / PostgreSQL 17 迁移与业务矩阵 | 通过，三库售价和毛利完全一致 |
| 前端类型检查 | 通过 |
| 定价控制台组件测试 | 通过，3 条 |

数据库复验覆盖旧结构升级、重复迁移和全新建库；三种数据库的建议售价均为 3.28125000/16.40625000，毛利均为 5342bps。真实上游账单比对仍属于生产验收项，需要有效的上游 API Key。

### 10.1 真实上游成本公式复验

使用本地 `.env` 中的 DDD 上游凭证完成一次受控调用，凭证未输出或写入报告：

| 项目 | 实测结果 |
|---|---|
| 上游模型列表 | HTTP 200，共 40 个模型 |
| 调用模型 | `deepseek-v4-flash`（上游实际返回 `deepseek-v4-flash-ga-260731`） |
| 请求结果 | HTTP 200，prompt 88 / completion 10 |
| 调用前累计用量 | 0.8544 |
| 调用后累计用量 | 0.8720 |
| 实际增量 | 0.0176，即 88 quota |

按已验证的分时表达式与 deepseek 分组倍率计算：

```text
基准 = (88 × 0.22 + 10 × 0.66) / 2 = 12.98 quota
真实成本 = 12.98 × 6.8 = 88.264 quota
上游按整数结算 = 88 quota
```

实测与公式一致。该结果验证了 `upstream_group_ratio` 必须进入采购成本计算的前提；OpenBridger 本地实现已由 EDGE-017 自动化用例验证。

### 10.2 单渠道完整闭环

使用 `/tmp/openbridger-single-e2e.db` 和本地端口 3211 启动隔离实例，完成“导入成本 → 生成方案 → 批准 → 发布 → 经本地网关调用 → 对比上下游账单”的完整流程：

| 项目 | 实测结果 |
|---|---|
| 成本导入 | 原始输入/输出成本 0.22/0.66，`upstream_group_ratio=6.8` |
| 含 5% 运营开销的预期成本 | 1.5708/4.7124 |
| 发布售价 | 2.41661538/7.24984615 |
| 目标毛利 | 3500bps（基于含运营开销成本） |
| 本地网关调用 | HTTP 200，prompt 88 / completion 11 |
| 本地用户扣费 | 146 quota |
| 上游真实扣费 | 91 quota |
| 按真实采购成本观察的毛利 | 3767bps；高于 3500bps 的部分来自 5% 运营开销和整数结算 |
| 消费日志 | 模型、146 quota、88/11 tokens 均与调用一致 |
| 发布后重新计算 | `PricingChanged=false`，输入/输出涨跌均为 0bps |

单渠道定价、发布、转发、扣费和审核台刷新闭环通过。主备切换与故障转移仍需要第二个上游渠道。
