...
复验对象：`origin/codex/fix-pricing-control-qa`

| 轮次 | 对象提交 | 记录章节 |
|---|---|---|
| 第 1 轮 | `fea1645ed` 修复 + `57fa756af` / `428ebb34d` 两份实测记录 | 第 1–5 节 |
| 第 2 轮 | `ddd054896 feat sync upstream pricing offers` | 第 6 节 |
| 第 3 轮 | 上线前三项确认核实（含三库矩阵补跑） | 6.9 节 |

复验时间：2026-09-18 ~ 2026-09-19　复验人：测试
被复验缺陷：《缺陷清单》5 项（`defect-list-pricing-control.md`）

> **阅读提示**：第 1–5 节是**第 1 轮的历史记录**，保留原始判断过程以便追溯。其中 1.5 / 2 / 3.1 的结论已被第 2、3 轮推翻或降级，**最终结论以第 0 节「最终状态」表为准**。

---

## 0. 结论

### 0.1 最终状态（第 3 轮，2026-09-19，HEAD `6144d8472`）

| 编号 | 缺陷 | 最终结论 | 状态 |
|---|---|---|---|
| CONC-001 | 报价过期后仍可批准方案 | 已修复，返回 `ErrPricingProposalStale` | ✅ 关闭 |
| CONC-002 | 价格版本变化后仍可批准方案 | 已修复，同上 | ✅ 关闭 |
| EDGE-001 | 零成本渠道产出 0 售价却报 35% 毛利 | 已修复，但预期行为被改写（报错而非保护价） | ⚠️ 已关闭，**需产品追认语义** |
| EDGE-013 | 批量导入非原子 | 已修复，非法批次后残留 0 条 | ✅ 关闭 |
| PC-COST-001 | 本地扣费不感知上游分组倍率 | 已修复，`ddd054896` 补上报价同步后链路闭环 | ✅ 关闭，可进生产验收 |
| RISK-001 | 倍率无生产方（第 1 轮阻断项） | 已解决：上游同步 + 定时任务 + 手工 API + 前端按钮 | ✅ 关闭 |
| RISK-002 | 存量报价升级即停摆 | 降级：upsert 冲突键会覆盖倍率，跑一次同步即回填 | ⚠️ 残留命名不一致场景 |
| RISK-003 | 单条写入绕过校验 / 错误码混用 | 低优先级，未处理 | 📌 遗留 |
| RISK-004 | 报价 2h 过期 + 定时任务默认不启用 | 需上线时配置环境变量（见 6.9 ①） | 📌 遗留，上线必做 |
| RISK-005 | 前端不展示同步 warnings | 未处理 | 📌 遗留 |
| 三库矩阵 | MySQL/PG 兼容性 | **本地实跑通过**（6.9 ③），三库数值一致 | ✅ 关闭 |
| 主备切换 | 故障转移验证 | 需第二个上游渠道，未做 | 📌 遗留 |

**整体结论：代码已达到可合入状态，可以走 PR。** 剩余为流程与遗留项，无阻断缺陷。

### 0.2 第 1 轮结论（历史记录，已过期）

> 以下为 2026-09-18 基于 `428ebb34d` 的判断，保留用于追溯。**已被 0.1 取代。**
...
另发现 **2 项新风险**（存量数据升级阻断、倍率无生产方），见第 2 节。
...
### 3.1 三库矩阵（MySQL / PostgreSQL）— 第 1 轮未执行，第 3 轮已补跑通过

> **状态更新（2026-09-19）**：本节的「未执行」结论已被 **6.9 ③** 推翻 —— 已用 mysql:8.0 + postgres:17 容器真跑，三库全部 PASS 且数值一致。**以下为第 1 轮原始记录，保留用于追溯「为什么当时不能信结论」。**
...
- [x] **产品确认** EDGE-001 的语义：零成本报价是「拒绝生成方案」还是「走保护价」→ **修复方选了「报错」，仍需产品追认**
- [x] **建事项** RISK-001：倍率采集入口或渠道默认倍率 → **`ddd054896` 已提供，闭环**
- [x] **建事项** RISK-002：存量报价回填脚本 → **不需要脚本**，跑一次同步即回填（`upsertUpstreamModelOffer` 冲突键覆盖倍率）
- [ ] **补回归用例**：存量 NULL 倍率场景（本次复验临时用例已验证有效，但未保留）
- [x] **补三库执行证据**：MySQL / PostgreSQL 上的加列与计算结果 → **6.9 ③ 本地实跑通过**，三库数值一致
- [x] **缺陷清单状态修正**：PC-COST-001 → **`ddd054896` 后已完全修复**，无需再标「部分修复」
- [ ] 生产验收时按 10.2 节闭环再跑一次：核对本地扣费与上游账单增量
- [x] 修复方后续如再改测试用例，请在提交信息中说明改动原因 → 第 2 轮（6.1）已说明，并新增 `TestQAPricingOfferSync`
- [ ] **新增（不再适用于跟进清单）**：3 个按次计费模型（图片生成）纳入动态定价的改造，已单独出规格 → `spec-per-request-pricing.md`
- [ ] **新增**：RISK-003 错误码语义混用、RISK-005 前端不展示 warnings
- [ ] **遗留**：主备切换与故障转移需第二个上游渠道

### 4.1 上线前必做（阻塞 deployment，非阻塞 merge）

1. 配置 `PRICING_CONTROL_ENABLED=true`（否则报价 2h 后全过期、动态定价停摆） → 详见 6.9 ①
2. 首次同步后核对 `warnings`，确认 3 个图片模型的处理方式 → 详见 6.9 ②
3. 三库矩阵已在本地验证，合入 PR 后确认 CI job 的日志里真的出现 `mysql` / `postgres` 两个 PASS → 详见 6.9 ③

---

## 5. 复验环境

| 项 | 第 1 轮（第 1–5 节） | 第 2/3 轮（第 6 节） |
|---|---|---|
| 分支 HEAD | `428ebb34d` | `6144d8472` |
| Go | go1.26.1 darwin/amd64 | 同左 |
| 数据库 | SQLite（`service` 包 TestMain 共享内存库） | SQLite + **MySQL 8.0(3399)** + **PostgreSQL 17(5499)** |
| 命令 | `go test ./service -run 'TestQAPricing\|TestPricing' -count=1` | `go test ./service -run '^TestQAPricingDatabaseMatrix$' -count=1 -v`，注入 `TEST_MYSQL_DSN` / `TEST_POSTGRES_DSN` |
| 结果 | `ok ... 7.178s`，5 个测试函数全部 PASS | `ok ... 3.639s`（单测）；三库矩阵 sqlite/mysql/postgres 全 PASS，售价 3.28125000 / 16.40625000、毛利 5342bps 三库一致 |
...
| 三库矩阵 | 未执行 | **已在本地实跑通过**（6.9 ③），数值一致；`t.Logf` 静默跳过的建议仍适用 |

> 说明：6.9 ③ 发布时本文此行写的是「CI 已补真跑；本地仍静默跳过」。后续本地补跑 mysql:8.0 + postgres:17 并通过，**该项已闭环**，此处同步更新。

**当前是否达到期待的结果：是。** 5 项缺陷全部修复，倍率链路闭环（上游 → 定时同步 → 成本计算 → 方案生成），三库兼容性已在本地实证。上线前建议确认：① `PRICING_CONTROL_ENABLED=true` 已配置；② 首次同步后核对 `warnings` 中 3 个图片模型的处理；③ 三库矩阵 —— **本项已不需要确认，见 6.9 ③**。
